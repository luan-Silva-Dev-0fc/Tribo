...
export function GroupDetailsScreen({
  ...
  const [activeTab, setActiveTab] = useState("chat"); // "chat" (padrão), "feed", "trends"
  ...
  const loadAppSettings = useCallback(async () => {
    try {
      ...
      // Default: feed e trends desativados (chat em tela cheia)
    }
  }, []);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      {/* Top Underline Tabs: Visível apenas se Feed ou Trends estiverem ativos */}
      ...
      {/* Modal do Menu de Opções do Grupo / Chat */}
      <Modal
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Lista de Opções com ScrollView de Segurança */}
            <ScrollView
              bounces={false}
            ...
            >
              {/* Opção 1: Configurações do Grupo */}
              <Pressable
                onPress={() => {
                  setGroupMenuVisible(false);
                  onSettings(group);
      ...
      {/* Modal Global Customizado para Avisos e Erros */}
      <CustomModal
...
function GroupFeedTab({
  ...
  // Add comment post state to show group comments modal
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            // Agrupamento Inteligente (Message Clustering)
            // Lembrete: FlatList está invertida (index + 1 é a mensagem cronologicamente anterior/acima, index - 1 é a posterior/abaixo)
            ...
            const isFirstInCluster = !isWithinTimeWithPrev; // Primeira mensagem do bloco (no topo)
            const isLastInCluster = !isWithinTimeWithNext; // Última mensagem do bloco (embaixo)
            ...
            return (
              ...
              >
                ...
                >
                  {isMe ? (
                    /* Mensagem do Usuário (Direita / Alto Contraste) */
                    <View
                      style={{
                        flexDirection: "row",
                        justifyContent: "flex-end",
                    ...
                    >
                      ...
                      >
                        {isBorderlessMedia ? (
                          /* Mídia / Figurinha sem caixa azul ao redor */
                          <View
                          ...
                          /* Balão de Mensagem de Texto / Áudio */
                          <View
                            style={{
                    ...
                    /* Mensagens Recebidas de Outros Membros (Esquerda) */
                    <View
                      style={{
                        flexDirection: "row",
                    ...
                    >
                      {/* Avatar renderizado com placeholder nos agrupados */}
                      {isLastInCluster ? (
                        <Pressable
                          onPress={() =>
                            onOpenProfile &&
                      ...
                      >
                        {/* Nome do autor exibido SOMENTE no topo do bloco de mensagens */}
                        ...
                        {isBorderlessMedia ? (
                          /* Mídia / Figurinha sem caixa de fundo ao redor */
                          <View
                            style={{ overflow: "hidden", borderRadius: 18 }}
                          >
                            {isViewOnce ? (
                          ...
                          /* Balão de Mensagem de Terceiros */
                          <View
                            style={{
                              backgroundColor: bubbleBg,
                              borderRadius: 18,
                          ...
                          >
                            {/* Mensagem Citada / Resposta (Outros) */}
                            {Boolean(resolvedReplyContext) && (
                              <QuotedMessageBlock
                                replyContext={resolvedReplyContext}
                                isMe={false}
...
const GroupChatTab = React.forwardRef(function GroupChatTab(
  ...
  const handleToggleLiveVoice = async () => {
    if (!isMeSpeaking) {
      ...
      // Inicia streaming contínuo de áudio do microfone para o WebSocket
      ...
      // Sair: Desconecta APENAS o usuário local sem derrubar os outros falantes
  ...
  const loadMessages = useCallback(async () => {
    try {
      ...
      // Garante ordem DESC (mais recente = índice 0) para FlatList inverted
  ...
  useEffect(() => {
    ...
    // Consulta inicial de falantes ativos ao entrar na tela
    ...
    try {
      ...
      if (recordingRef.current) {
        ...
        if (uri) {
          ...
          // 1. Inserção Otimista da Mensagem de Áudio na lista
          ...
          // 2. Upload do arquivo de áudio para o servidor
          const uploadFn =
            api.uploads?.audio ||
            api.upload?.audio ||
            api.uploads?.media ||
          ...
          // 3. Envio da mensagem final para a Tribo
          ...
          // 4. Substituir a mensagem otimista pelo registro definitivo do servidor
      ...
      // Reverte a mensagem otimista em caso de falha no upload/envio
      setMessages((prev) =>
  ...
  return (
    ...
    >
      {/* === BANNER FIXO: VOZ AO VIVO NA TRIBO (Sticky, multi-falante) === */}
      <Pressable
      ...
      {loading && messages.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            // Agrupamento Inteligente (Message Clustering)
            // Lembrete: FlatList está invertida (index + 1 é a mensagem anterior/acima, index - 1 é a posterior/abaixo)
            const prevItem = messages[index + 1];
            const nextItem = messages[index - 1];
            ...
            const isFirstInCluster = !isWithinTimeWithPrev; // Primeira mensagem do bloco
            const isLastInCluster = !isWithinTimeWithNext; // Última mensagem do bloco
            ...
            // Paleta de Cores de Alto Contraste
            // Mensagens enviadas: Azul vibrante #0284c7 (ou #2563eb / #1e293b para áudio)
            const bubbleBg = isMe
              ? isAudio
                ? "#1e293b"
                : colors.primary || "#0284c7"
            ...
            return (
              ...
              >
                ...
                >
                  {isMe ? (
                    /* Mensagem do Próprio Usuário (Direita - Sem Avatar e Sem Label 'Você') */
                    <View
                      style={{
                        flexDirection: "row",
                        justifyContent: "flex-end",
                    ...
                    >
                      ...
                      >
                        {isBorderlessMedia ? (
                          /* Mídia / Figurinha sem caixa ao redor */
                          <View
                          ...
                          /* Balão de Mensagem de Texto / Áudio / Mídia */
                          <View
                            style={{
                    ...
                    /* Mensagens de Outros Membros (Esquerda) */
                    <View
                      style={{
                        flexDirection: "row",
                        justifyContent: "flex-start",
                    ...
                    >
                      {/* Avatar exibido apenas na última mensagem do bloco ou espaçador para alinhamento */}
                      {isLastInCluster ? (
                        <Pressable
                          onPress={() =>
                            onOpenProfile &&
      ...
      {/* Barra de Pré-Visualização de Resposta */}
      {!isBanned && (
        <ReplyPreviewBar
      ...
      {/* Trava Visual para Usuários Banidos OU Bottom Composer Normal */}
      {isBanned ? (
        <View
          style={{
            marginHorizontal: 14,
        ...
        /* Bottom Composer - Posicionamento Confortável e Moderno */
        <View
          style={{
            flexDirection: "row",
            alignItems: "center",
        ...
        >
          {isRecording ? (
            ...
            /* Normal TextInput Capsule - Redesign Moderno */
            <View
              style={{
                flex: 1,
                flexDirection: "row",
      ...
      {/* Modal de Confirmação de Exclusão Moderno */}
      <ConfirmDeleteModal
      ...
      {/* Modal de Inventário de Figurinhas */}
      <StickerPickerModal
      ...
      {/* Modal Customizado de Alerta Interno */}
      <CustomModal
      ...
      {/* Visualizador de Fotos e Vídeos em Tela Cheia */}
      <MediaViewerModal
...
function GroupComments({
  ...
  return (
    ...
    >
      ...
      {/* Modal Moderno de Confirmação para Excluir Comentário do Grupo */}
      <Modal
      ...
      {/* Modal de Alerta para Sucesso/Erro */}
      <TriboAlertModal
